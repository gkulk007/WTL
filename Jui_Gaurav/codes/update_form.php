      ...
      <form action="update_form.php" method="POST" class="contact-form form-validate" novalidate="novalidate">
        <div class="row">
          ...
          <div class="col-sm-12 mb-3 text-center">
            <button type="button" name="submit" class="btn btn-lg btn-success"><a href="cards.php">View all Developers</a></button>
...
if (isset($_POST['submit'])) {
    if (!empty($_POST['firstName']) && !empty($_POST['lastName']) && !empty($_POST['email']) && !empty($_POST['phone']) && !empty($_POST['Github']) && !empty($_POST['linkedin'])) {
        ...
        $query = "UPDATE devs SET firstName='$firstName', lastName= '$lastName', Github='$Github', linkedin='$linkedin' WHERE email='$email' AND phone='$phone'";
